...
export function calculateBlobStats(body: Matter.Body, strokes: Stroke[]): BlobStats {
  // Get vertices from the Matter.js body
  const vertices = body.vertices.map(v => ({ x: v.x, y: v.y }));
  
  // Calculate area from body vertices (Shoelace formula)
  ...
  // Apply calculated mass to the Matter.js body
  ...
  const damage = STATS.BASE_DAMAGE +
...
export function previewStats(strokes: Stroke[]): Partial<BlobStats> {
  ...
  const approxArea = (maxX - minX) * (maxY - minY) * 0.6; // ~60% fill estimate
