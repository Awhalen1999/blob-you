...
function getPostHog(room: Party.Room): PostHog | null {
  const env = room.env as Record<string, string | undefined>;
  const key = env.POSTHOG_API_KEY ?? env.NEXT_PUBLIC_POSTHOG_KEY;
  if (!key) return null;
  if (!_posthog) {
    _posthog = new PostHog(key, { host: env.NEXT_PUBLIC_POSTHOG_HOST ?? 'https://us.i.posthog.com' });
  }
  return _posthog;
}
...
export default class BlobRoom implements Party.Server {
  ...
  private getStkClient(): Client {
    if (!this.stkClient) {
      ...
      const token = env.STACKCOIN_BOT_TOKEN;
