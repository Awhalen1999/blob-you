- ensure old rooms are deleted / investigate
- update UI for ready state in drawing canvas
- When connected add chip on bottom right to show other player in room and update if leaves room
- fix logic for battle mode logic (offline mode vs online mode)
- update match code / match room UI slightly
- add leave icon to leave room button
- make sure room gets killed or return to room page after opp leaves

Nice to haves:
- animated blob for main menu
- chat
- sharpness nerf
- bomb/damaging power up?
